...
3. **Set your API key** — either:
   ...
   (useful for quickly trying it out without env vars).

4. **Run the Streamlit app (teacher dashboard):**
...
5. **Run the REST API** (optional — for other apps/websites to call):
...
### Two different paths depending on your hardware

- **NVIDIA GPU server (A100/H100/etc.)** → use **vLLM** (Step 1 below).
  This is the production path: highest throughput, supports serving
  multiple models, LoRA adapters, many concurrent users.
- **Apple Silicon Mac (M1/M2/M3, unified memory)** → vLLM does not run
  here at all (it requires NVIDIA CUDA). Use **Ollama** instead — this is
  a genuinely different tool, not a smaller version of vLLM. It's the
  right choice for learning/prototyping on a laptop, not for serving many
  students at once.

#### Quick start on a MacBook (Ollama) — for learning/prototyping only

```bash
# 1. Install Ollama
brew install ollama

# 2. Start it (or it may already be running as a menu-bar app after install)
ollama serve

# 3. In another terminal, pull a vision-language model.
#    Start small to confirm everything works:
ollama pull qwen2.5vl:7b
#    Once that works, try a stronger one -- 64GB unified memory comfortably
#    fits a 32B model at typical quantization:
...
available for Apple Silicon yet, so this avoids a dead end there.

**Honest limits of this path**: a laptop is fine for trying things out and
building intuition, but it is not a real "server" — it has far less raw
compute than an A100/H100, needs to stay awake and on the same network as
whoever's using the app, and can only handle one or a few requests at a
time before things slow down noticeably. Treat this as how you'd learn
and prototype the workflow, not as infrastructure for real students at
scale. When you're ready for that, the vLLM path below (on a rented or
owned NVIDIA GPU server) is the one to use — everything else about the
app (config, `local_checker.py`, the rest of this guide) stays identical,
you're only swapping which server `LOCAL_MODEL_BASE_URL` points to.
